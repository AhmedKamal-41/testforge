...
| Backend (pytest) | 10 | n/a | FastAPI via TestClient on SQLite |
| Backend (Postgres) | 6 | `@postgres` | Transactions and concurrency on real Postgres |
| **Total** | **120** | | |
| k6 (optional) | 4 scripts | n/a | Light load on hot API paths |
...
1. **Terminal list** for live progress  
2. **HTML report** in `qa/e2e/playwright-report/`  
3. **Allure raw results** in `qa/e2e/allure-results/`  
...
- Each Playwright CI job starts its own Compose stack. That keeps jobs isolated but is not the fastest pipeline.
